<template>
  <div class="stockTree"
    ...
    <div class="firm_stockInfo_content_title">全部下级</div>
...
export default {
  ...
  mounted () {
    ...
    document.getElementsByClassName("org-tree-container")[0].setAttribute('style', 'min-width:'+ 10000+'rem');
    this.$nextTick(()=>{
      widthSmall = document.querySelector(".org-tree").offsetWidth;
      widthBig = document.querySelector(".firm_stockInfo_content").offsetWidth;
      if(that.treeWidth < 900){
    ...
    let urls = dataUrl.url +'/EntinvInfo/getEntinvAllSubordinateInfo';
    var qs = require('qs');
    let config = {
        headers: {
